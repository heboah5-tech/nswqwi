...
import { Menu, Search, ShoppingCart, Heart, Phone, User, LogOut } from "lucide-react";
      ...
      <header className={`bg-card border-b border-border transition-all duration-300 ${scrolled ? "shadow-lg py-2" : "py-3"}`}>
        <div className="max-w-7xl mx-auto px-4 flex items-center justify-between gap-6">
          ...
          <div className="hidden lg:flex flex-1 max-w-xl relative group">
            ...
            <button className="absolute left-1.5 top-1.5 bottom-1.5 bg-primary text-primary-foreground px-4 rounded-full text-xs font-bold hover:bg-primary/90 transition-all">
              بحث
            </button>
          </div>
              ...
              {isLoaded && (
                isSignedIn ? (
                  <div className="hidden sm:flex items-center gap-2">
                    <span className="text-sm text-foreground font-medium max-w-[120px] truncate">{user.firstName || user.emailAddresses[0]?.emailAddress}</span>
                    <button
                  ...
                  <button
                    ...
                    className="p-2.5 hover:bg-secondary rounded-full transition-colors text-foreground hidden sm:block"
                    title="تسجيل الدخول"
                  >
